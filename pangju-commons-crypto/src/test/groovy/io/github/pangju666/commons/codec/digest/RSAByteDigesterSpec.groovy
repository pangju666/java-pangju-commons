...
import io.github.pangju666.commons.crypto.digest.RSAByteDigester
...
class RSAByteDigesterSpec extends Specification {
	...
	def "Digest"() {
		...
		def digestStr = Base64.encodeBase64String(digest)

		then:
		digestStr == expectDigest
	...
	def "Matches"() {
		...
		when:
		def result = digester.matches(message.getBytes(), Base64.decodeBase64(digest))
