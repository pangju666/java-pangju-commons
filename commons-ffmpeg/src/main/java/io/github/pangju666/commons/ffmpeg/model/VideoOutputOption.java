...
public class VideoOutputOption extends OutputOption {
	...
	public VideoOutputOption(Video video) {
		...
		setFormat(FFmpegUtils.parseFormat(video.format()));
		Validate.notBlank(format, "format 不可为空");

		setCodecId(video.codecId());
		setFrameRate(video.frameRate());
		setImageWidth(video.imageWidth());
		setImageHeight(video.imageHeight());
		setBitrate(video.bitrate());
		// 不提取像素格式
		//setPixelFormat(video.pixelFormat());
		if (Objects.nonNull(video.audio())) {
			setAudio(new AudioOutputOption(video.audio()));
		}
	}
