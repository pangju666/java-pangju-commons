...
/**
 * Gson 类型适配器：在 JSON 与 {@link LocalTime} 之间转换。
 * <p>
 * 序列化：输出为 ISO-8601 字符串（例如 {@code 12:34:56.789}）。<br>
 * 反序列化：支持的输入 token：
 * <ul>
 *   <li>{@link JsonToken#NULL}：返回 {@code null}</li>
 *   <li>{@link JsonToken#STRING}：使用 {@link LocalTime#parse(CharSequence)} 解析 ISO-8601 字符串</li>
 *   <li>{@link JsonToken#NUMBER}：将毫秒数按系统默认时区 {@link ZoneId#systemDefault()} 转换为 {@link LocalTime}</li>
 *   <li>其他类型：抛出 {@link JsonParseException}</li>
 * </ul>
 * </p>
 *
 * <h3>时区语义</h3>
 * <p>数字毫秒在反序列化时依据系统默认时区转换为本地时间。</p>
 *
 * <h3>线程安全</h3>
 * <p>类无状态，可在多线程中安全复用。</p>
 *
...
public class LocalTimeTypeAdapter extends TypeAdapter<LocalTime> {
	...
	public LocalTime read(JsonReader in) throws IOException {
		...
		if (token == JsonToken.NUMBER) {
			return LocalTime.ofInstant(Instant.ofEpochMilli(in.nextLong()), ZoneId.systemDefault());
		}
