...
/**
 * Gson 类型适配器：在 JSON 与 {@link BigInteger} 之间转换。
 * <p>
 * 序列化：输出为十进制字符串（使用 {@link BigInteger#toString()}）。<br>
 * 反序列化：支持的输入 token：
 * <ul>
 *   <li>{@link com.google.gson.stream.JsonToken#NULL}：返回 {@code null}</li>
 *   <li>{@link com.google.gson.stream.JsonToken#STRING}：使用 {@link BigInteger#BigInteger(String)} 解析，格式非法时返回 {@code null}</li>
 *   <li>{@link com.google.gson.stream.JsonToken#NUMBER}：读取为长整型并转换为 {@link BigInteger}（{@link BigInteger#valueOf(long)})</li>
...
public class BigIntegerTypeAdapter extends TypeAdapter<BigInteger> {
	/**
	 * 将 {@link BigInteger} 写出为 JSON。
	 * <p>输出为十进制字符串；当值为 {@code null} 时输出 JSON {@code null}。</p>
	 *
	 * @param out   JSON 写出器（由 Gson 管理其生命周期）
	 * @param value 待写出的 {@link BigInteger} 值
	...
	/**
	 * 从 JSON 读取为 {@link BigInteger}。
	 * <p>
	 * 支持：
	 * <ul>
	 *   <li>NULL → {@code null}</li>
	 *   <li>STRING → {@link BigInteger}（字符串格式非法时返回 {@code null}）</li>
	 *   <li>NUMBER（长整型） → {@link BigInteger#valueOf(long)}</li>
	 * </ul>
	 * 其他 token 类型抛出 {@link JsonParseException}。
	 * </p>
	 *
	 * @param in JSON 读取器（由 Gson 管理其生命周期）
	 * @return 解析得到的 {@link BigInteger} 或 {@code null}
