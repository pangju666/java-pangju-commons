...
/**
 * BigInteger 类型的 Gson 自定义反序列化器
 * <p>
 * 该反序列化器用于将 JSON 元素转换为 BigInteger 对象。支持从以下类型的 JSON 元素进行转换：
 * <ul>
 *     <li>数字类型：使用 {@link BigInteger#valueOf(long)} 进行转换</li>
 *     <li>字符串类型：使用 {@link BigInteger#BigInteger(String)} 构造函数进行转换</li>
 * </ul>
...
public class BigIntegerDeserializer implements JsonDeserializer<BigInteger> {
	/**
	 * 将 JSON 元素反序列化为 BigInteger 对象
	 * <p>
	 * 反序列化逻辑：
	 * <ol>
	 *     <li>检查 JSON 元素是否为基本类型（{@link JsonPrimitive}），如果不是则返回 null</li>
	 *     <li>如果是数字类型，则使用 {@link BigInteger#valueOf(long)} 进行转换</li>
	 *     <li>如果是字符串类型，则尝试使用 {@link BigInteger#BigInteger(String)} 构造函数进行转换</li>
	 *     <li>如果字符串无法转换为有效的 BigInteger（抛出 {@link NumberFormatException}），则返回 null</li>
	 ...
	 * @return 转换后的 BigInteger 对象，如果无法转换则返回 null
