...
public class CompressUtils {
	...
	public static void compress(final IOResource resource, final File outputFile) throws IOException {
		...
		switch (outputFormat) {
			case "gz":
				GzipUtils.compress(resource, outputFile);
				break;
			case "xz":
				XZUtils.compress(resource, outputFile);
				break;
			case "zst":
		...
		switch (outputFormat) {
			...
			case "tar.gz":
				...
				try (InputStream inputStream = FileUtils.newUnsynchronizedBufferedInputStream(gzTarFile)) {
					...
					FileUtils.forceDeleteIfExist(gzTarFile);
				}
				break;
			case "txz":
			case "tar.xz":
				...
				try (InputStream inputStream = FileUtils.newUnsynchronizedBufferedInputStream(xzTarFile)) {
					...
					FileUtils.forceDeleteIfExist(xzTarFile);
				}
				break;
			case "tzst":
			case "tar.zst":
