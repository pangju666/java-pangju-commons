...
public class ImageUtils {
	...
	public static boolean isSupportImageType(final byte[] bytes) throws IOException {
		...
		try (ImageInputStream imageInputStream = ImageIO.createImageInputStream(bytes)) {
			return Objects.nonNull(getImageType(imageInputStream));
		}
	}
	...
	public static boolean isSameImageType(final String mimeType1, final String mimeType2) {
		...
		reader.dispose();
		return ArrayUtils.contains(readerSpi.getMIMETypes(), mimeType2);
	}
	...
	public static ImageSize getImageSize(final byte[] bytes) throws IOException {
		...
		return parseImageSize(IOUtils.toUnsynchronizedByteArrayOutputStream(bytes));
	}
	...
	public static ImageSize getImageSize(final InputStream inputStream) throws IOException {
		Validate.notNull(inputStream, "inputStream 不可为 null");
		return parseImageSize(IOUtils.toUnsynchronizedByteArrayOutputStream(inputStream));
	}
	...
	protected static String[] parseImageTypes(final ImageInputStream imageInputStream) {
		...
		if (readers.hasNext()) {
			...
			if (Objects.nonNull(readerSpi)) {
				return ArrayUtils.nullToEmpty(readerSpi.getMIMETypes());
			}
		}
	 ...
	 * <p>处理流程：</p>
	 * <ol>
	 *   <li>优先通过元数据解析尺寸（支持方向校正）</li>
	 *   <li>失败后使用ImageIO获取原始尺寸</li>
	 *   <li>自动管理图像输入流资源</li>
	 * </ol>
	 *
	 * @param outputStream 包含图像数据的字节输出流
	 * @return 图像尺寸，解析失败返回null
	 * @throws IOException 当流操作失败时抛出
	 * @since 1.0.0
	 */
	protected static ImageSize parseImageSize(UnsynchronizedByteArrayOutputStream outputStream) throws IOException {
		try {
