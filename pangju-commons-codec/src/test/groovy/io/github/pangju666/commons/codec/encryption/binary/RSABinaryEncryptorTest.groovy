...
class RSABinaryEncryptorTest extends Specification {
	RSAKey key
	...
	def "test"() {
		...
		when:
		byte[] bytes = encryptor.encrypt(((String) message).getBytes())
		def result = new String(encryptor.decrypt(bytes))
	...
	def "test2"() {
		given:
		def encryptor = new RSABinaryEncryptor()
		encryptor.setAlgorithm("RSA/ECB/OAEPPadding")
		encryptor.setKey(key)
		encryptor.initialize()
